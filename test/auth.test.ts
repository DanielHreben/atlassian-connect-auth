...
describe('Auth', () => {
  test('Missing token', async () => {
    ...
    await expect(jiraAddon.auth(req, { loadCredentials: noop })).rejects.toMatchError(
      new AuthError('Missed token', AuthErrorCodes.MISSED_TOKEN)
    );
  });
  ...
  test('Unknown issuer', async () => {
    ...
    ).rejects.toMatchError(new AuthError('Unknown issuer', AuthErrorCodes.UNKNOWN_ISSUER));
  ...
  test('Token expired', async () => {
    ...
    await expect(jiraAddon.auth(req, { loadCredentials })).rejects.toMatchError(
      new AuthError('Token expired', AuthErrorCodes.TOKEN_EXPIRED)
    );
    expect(loadCredentials).toHaveBeenCalledWith(req.body.clientKey);
  });
  ...
  test('Invalid QSH', async () => {
    ...
    ).rejects.toMatchError(new AuthError('Invalid QSH', AuthErrorCodes.INVALID_QSH));
    expect(loadCredentials).toHaveBeenCalledWith(req.body.clientKey);
  });
  ...
  test('Passed Node.js HTTP request object', async () => {
    ...
    const result = await jiraAddon.auth(req, {
      loadCredentials: async () => jiraPayload,
    });
  ...
  test('Extract token from a custom place', async () => {
    ...
    const result = await jiraAddon.auth(req, {
      loadCredentials: async () => jiraPayload,
      customExtractToken: () => req.query.state as string,
      skipQsh: true,
    });
